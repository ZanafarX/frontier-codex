---
name: "Circle Initiate"
creature_type: "human"
faction: "circle-of-ash"
tier: "minion"
xp_value: 1
vitals:
  hp: 6
  defense: 8
  armor_rating: 0
  mettle: 0
  speed: 6
attributes:
  echo: 2
attacks:
  - name: "Ritual Knife"
    pool: 3
    weapon_rating: 2
    tempo: 3
    qualities: []
traits:
  - "**Bookworm:** Initiates know things. If the party needs obscure historical, magical, or geographic information, a captured Initiate can provide it with a successful SWAY + Persuasion check (TN 9 — they're not brave)."
abilities:
  - "**Channel Pool 4 (ECHO 2, Channel 2).** Known Talents:"
  - "- *Glimpse* (R1): After any ally (including self) rolls, spend 1 Strain to reroll one die. Must accept the new result."
habitat:
  - "ash-belt"
  - "graven-plain"
  - "stormrise"
---

Scholars who traded the relative safety of ignorance for the
dangerous enlightenment of forbidden knowledge. Initiates are the
Circle's students: researchers, librarians, and apprentices who
spend their days cataloguing pre-Rupture texts and their nights
trembling at what they've read.

Most Initiates are young and idealistic, drawn to the Circle by the
promise of answers. They wear the grey robes of scholars and carry
ritual knives they have never used on anything more threatening than
a candle wick.

Gear: Grey robes, ritual knife, leather satchel of notes, ink and
quill, candle. Speed 6.

## Tactics

Stay behind Guardians. Use Glimpse to help allies avoid critical
failures. **Surrender immediately if threatened directly**: their
knowledge is more valuable than their pride.


## Morale

Fragile (−2). Initiates are scholars, not soldiers. They break easily
and offer information freely.
