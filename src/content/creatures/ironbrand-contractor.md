---
name: "Ironbrand Contractor"
creature_type: "human"
faction: "ironbrands"
tier: "soldier"
xp_value: 2
vitals:
  hp: 9
  defense: 9
  armor_rating: 1
  mettle: 1
attacks:
  - name: "Repeater"
    pool: 5
    weapon_rating: 4
    tempo: 5
    qualities: ["rapid-cycle"]
  - name: "Combat Knife"
    pool: 5
    weapon_rating: 2
    tempo: 3
    qualities: []
traits:
  - "**Professional:** +2 to resist Fear, Intimidation, and bribes under §50. They have been offered better by worse."
  - "**Contract-Bound:** An Ironbrand Contractor will not betray their current contract for any reason short of the contract holder breaking its terms first."
abilities: []
habitat: []
---

The backbone of the guild — professional guns-for-hire who ask no
questions and answer to the contract. Contractors are veterans of at
least two years' service, which on the Frontier means they have
survived things that would break most people. They are calm,
competent, and relentlessly practical.

An Ironbrand Contractor can be identified by the **Anvil Brand** on
their left forearm — a stylized anvil seared into the skin during
their oath ceremony. It cannot be forged. It cannot be removed
without taking the arm.

Gear: Boiler Plate (AR 1, Hardened), binoculars, signal flares ×2,
ration pack, contract badge. Speed 7.

## Tactics

Disciplined fire-and-maneuver. Use cover, coordinate with squadmates,
maintain spacing. They won't break contract but will execute a
tactical withdrawal if outmatched — retreating isn't losing, it's
repositioning.

## Morale

Professional (+2). Will not break while under contract. Will retreat
to regroup if the situation is untenable, then return with
reinforcements.
