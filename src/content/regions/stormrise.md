---
name: "The Stormrise Bluffs"
short_name: "stormrise"
tagline: "Where Lightning Walks"
terrain: "Jagged highlands, granite cliffs, and perpetual storm clouds. The sky is a permanent bruise of purple and orange. Thunder is constant — you feel it in your teeth."
hazards:
  - name: "The Metal Rule"
    description: "While wearing heavy metal armor (AR 2+) during storms, roll 1d6 every hour. On a 1, a lightning strike inflicts 4d6 damage (usually fatal). Mitigation: remove armor or seek shelter."
  - name: "The Hum"
    description: "Static charge saturates the air. All Psionic Focus checks suffer −2 dice (Pool Penalty). Hair stands on end; metal tastes sour."
  - name: "Acoustics of the Dead"
    description: "Canyon walls carry sound impossibly far. Whispers travel miles. Base Stealth TN is 15 (sound echoes everywhere). Gunfire attracts attention from great distances."
settlements: []
landmarks:
  - "red-mothers-teeth"
---

The highest elevation in the Frontier and the most volatile. The sky
is a permanent bruise of purple and orange. Thunder is constant; you
feel it in your teeth.

No single faction controls the Stormrise. Independent settlements
cling to its bluffs. **Circle of Ash** outposts study the Ley-Line
crossings that warp reality here. The Consortium has abandoned every
attempt to lay rail through this region; the storms eat infrastructure
faster than crews can build it.

## Threats & Features

| Feature | Effect |
| :------ | :----- |
| Stormcallers | Elemental riders of blue lightning. Hunt anything moving fast. Walk slowly to avoid. |
| Flash Floods | Not water: static discharge and fist-sized hail. GRIT (TN 13) or take 2d6 damage. |
| Ley-Line Crossings | Reality breaks. Gravity at 80%. Time moves strangely. |

## Settlements

Source names **Lathrop**, built directly on a massive Ley-Line
crossing where gravity is 80% normal and people age differently
depending on which side of the street they live on. No settlement file
exists yet; none of the 6 planned settlements are in this region.

## Landmarks

**Red Mother's Teeth**, the crimson cliffs that bleed during
lightning strikes, is the currently-filed landmark for this region.
