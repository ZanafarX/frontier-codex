---
type: "psionic"
name: "Hivemind Pulse"
discipline: "flesh-shaper"
rank: 4
rank_name: "Devastation"
cost:
  erosion: 2
tn: 15
tempo: 7
flavor: "Every nervous system in range becomes an extension of mine."
tags: ["aura", "team-buff", "investigation", "perception"]
has_steps: true
has_resist: false
---

## Effect

You extend your biological awareness to encompass every living creature
within Near range. For **10 Ticks**:

- You feel the **precise location, health state, and emotional state**
  of every creature
- You can **communicate biologically** (not telepathically, through
  shared nerve impulses) with willing creatures, granting **+2 dice**
  (Pool Bonus) to coordinated actions
- You can **identify Skin-Changers, Veil-Born in disguise, and
  non-living entities** by their absence from the biological web (they
  feel "hollow" in the pulse)
- You can deliver **Rank 1 Flesh Shaper Talents** to any creature in
  the web **without touching them** (the nerve web carries the effect)

## Steps

+5 Ticks duration per Step.

## Special

You are the **hub**. If you are knocked unconscious, the web collapses.
**Bastion Souls within the area create "dead zones" in the web**: you
sense them as cold spots where the pulse doesn't reach. This is sensory
only; no Bastion Soul Talent is required to create the dead zone, just
their presence.
